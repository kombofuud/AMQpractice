...
$("#cslgAnisongdbModeSelect").val("Song");
...
function setup() {
    ...
    new Listener("New Spectator", (payload) => {
        if (quiz.cslActive && quiz.inQuiz && quiz.isHost) {
            ...
            setTimeout(() => {
                ...
                let message  = `${currentSong}§${getStartPoint(song.startPoint)}§${song.audio || ""}§${song.video480 || ""}§${song.video720 || ""}`;
    ...
    document.body.addEventListener("keydown", (event) => {
        ...
        if (event.keyCode == '222' && event.shiftKey && ctrlKey){
            if (quiz.cslActive) {
                if (quiz.soloMode) {
                    if (quiz.pauseButton.pauseOn) {
                        fireListener("quiz unpause triggered", {
...
function startQuiz() {
    ...
    setTimeout(() => {
        if (quiz.soloMode) {
            ...
            if (quiz.isHost) {
                let message  = `1§${getStartPoint(song.startPoint)}§${song.audio || ""}§${song.video480 || ""}§${song.video720 || ""}`;
    ...
    setTimeout(() => {
        if (songNumber < totalSongs) {
            if (quiz.soloMode) {
                ...
                if (quiz.isHost) {
                    let nextSong = songList[songOrder[songNumber + 1]];
                    let message  = `${songNumber + 1}§${getStartPoint(nextSong.startPoint)}§${nextSong.audio || ""}§${nextSong.video480 || ""}§${nextSong.video720 || ""}`;
                    splitIntoChunks(btoa(encodeURIComponent(message)) + "$", 144).forEach((item, index) => {
                        cslMessage("§CSL3" + base10to36(index % 36) + item);
                    });
                }
