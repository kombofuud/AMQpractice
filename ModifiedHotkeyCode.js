...
function doc_keyUp(event) {
	...
	else if(false && event.keyCode=='77' && event.ctrlKey) {
...
function doc_keyDown(event) {
	if(event.shiftKey) {
        ...
        if(event.keyCode=='20' && !event.altKey) {
        ...
        else if(event.keyCode=='188' && !quiz.isSpectator && event.ctrlKey && justPressed) {
            justPressed = false;
            quiz.skipClicked();
        }
	}
