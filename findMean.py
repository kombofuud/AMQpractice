...
for song in known:
    if song["D"] <= 0:
        maxWeightCount += 1
    DList.append(2/(1+math.exp(song["D"]))*math.sqrt(len(song["sampleWeights"])))
...
maxCount = -1
songTotal = 0
songDistribution = [0]*(int(math.ceil(DMax))-int(math.ceil(DMin))+1)
for _ in range(1000):
    ...
    for song in randomSongList:
        songCount += 1
        songDistribution[int(math.ceil(song["D"]))-int(math.ceil(DMin))] += 1
        if song["D"] <= 0:
            testWeightCount-= 1
